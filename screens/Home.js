...
const Home = () => {
  ...
  const renderContent = () => {
    if (selectedCryptos.length > 0) {
      ...
      return selectedCryptos.map((crypto, index) => (
        <TouchableOpacity key={index} style={styles.content} onPress={() => setshowCurrencyChart(true)}>
          <View>
            <Text
              style={{
                fontFamily: 'SF-Pro-Text-Bold',
  ...
  if (showCurrencyChart) {
    return <LiveChart tickerValues={tickerValues} selectedCryptos={selectedCryptos} Back={handlebackhome} />
  } else {
    return (
      <ImageBackground
        source={require('../assets/Dashboardbg.png')}
